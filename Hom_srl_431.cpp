...
/*void C_Hom_Doc::Serial_Retrieve_v431_a ( CArchive &aArch )
{
   this->Serial_Retrieve_v431_b( aArch );

   //--- Additional 32-2 members
   Factors lFactors;
   aArch >> lFactors;
   this->SetMaleFactors(lFactors);
   // Because the actual and standard values are seperated in 
   lFactors.mActualValue = lFactors.loadedFemaleActVal;
   lFactors.mStdDev = lFactors.loadedFemaleStdDev;
   ...
   WindowData dummyWD;
   aArch >> dummyWD;	// Previously mCOPWind - removed in v4.40
   ...
   aArch >> dummyBOOL;	//this->mReportBalanceStanding;
...
void	  //------------------------------
C_Hom_Doc::Serial_Retrieve_v431_b ( CArchive& Arch )
{
   long Signed_Int;
   ...
   Arch >> Signed_Int;  // Formerly this->Obl_Light_Type (pre-440)
   ...
   Arch >> Signed_Int;	// Used to be this->Ort_Render_Type (pre-440)
   ...
   Arch >> Signed_Int;  // Used to be this->Ort_Light_Type (pre-440)
   ...
   //### Saved UCS Name Strings -> (mpt) obsolete, load into dummy
   CString dummy;
...
//----------------------------------------------------------------------------
void C_Hom_Doc::ConvertLightType (	int aOldLightType ,
								  int	&aLightTypeFB ,
								  int &aLightTypeLR ,
								  int &aLightTypeTB)
								  //NOTE: Old convention left/right was with respect to the viewer. 
								  //The new convention is with respect to the subject, hence the
								  //reversal of left and right below.
{
	assert( aOldLightType >= (int)LT_TR );
	assert( aOldLightType <= (int)LT_BL );
	
	switch( aOldLightType )
	{
	case LT_TR:
		{
			aLightTypeFB =  0;
	...
	default:
		{
			AfxMessageBox( "C_Hom_Doc::ConvertLightType: Unknown light type." );
		} 
	}
	return;
}
