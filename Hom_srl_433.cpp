...
/*void C_Hom_Doc::Serial_Retrieve_v433_a ( CArchive &aArch )
{
   float tempFloat;  // For reading in older values with less precision

   this->Serial_Retrieve_v433_b( aArch );
   ...
   WindowData dummyWD;
   aArch >> dummyWD; // mCOPWind removed in v4.40
   aArch >> dummyWD; // mSeatCOPWind removed in 4.40

   aArch >> this->mObliqueWindowData;
   aArch >> this->mStatusResultsWindow;

   // ACT::ADDED [05/03/01]- Special Seating Conditions
   aArch >> tempFloat; //this->mDeltaX = tempFloat;
   aArch >> tempFloat; //this->mDeltaY = tempFloat;
   aArch >> tempFloat; skeleton->SetAngle(A_PelvisFlexion, tempFloat);//setPelvicTilt(tempFloat);

   aArch >> this->mGravity;   // ACT - For BOEING
...
//--------------------------------------------------------------
void C_Hom_Doc::Serial_Retrieve_v433_b ( CArchive& Arch )
{
   long Signed_Int;
